import { useState, useEffect } from "react";

import Foreigner from "./Foreigner";
import MobileForeigner from "./MobileForeigner";

function Pricing() {
  const [screenSize, setScreenSize] = useState(window.innerWidth);

  useEffect(() => {
    const handleResize = () => {
      setScreenSize(window.innerWidth);
    };
    window.addEventListener("resize", handleResize);
    return () => {
      window.removeEventListener("resize", handleResize);
    };
  }, []);

  return (
    <>
      <section className="overflow-hidden px-[3.5rem] lg:px-[5rem] pt-[2rem] pb-[3rem] md:pt-[3.5rem]">
        <h1 className="font-[800] text-[3rem] text-center leading-[3.5rem]">
          Transparent pricing
        </h1>
        <p className="hidden md:block text-[#262626] mt-[1rem] text-[1.3rem] text-center">
          On the “Regular” plan you will get the maximum savings,
          <br /> on "Plus" there will be an opportunity to reschedule and cancel
          lessons,
          <br /> and on "Flexi" you will be accompanied by a mentor.
        </p>
        <p className="md:hidden text-[#262626] mt-[1rem] text-[1.3rem] text-center">
          “Regular” plan will get savings.
          <br /> On "Plus" you can reschedule and cancel lessons.
          <br /> On "Flexi" you will be accompanied by a mentor.
        </p>

        <div className="md:hidden flex justify-center mt-[2rem] text-center">
          <div className="cursor-pointer border-[3px] w-[10rem] py-[0.5rem] rounded-l-full border-[#CE4A37]">
            Foreigner
          </div>
          <div className="cursor-pointer border-[3px] w-[10rem] py-[0.5rem] rounded-r-full opacity-50 border-[#909090]">
            Native
          </div>
        </div>
        {screenSize >= 768 ? <Foreigner /> : <MobileForeigner />}
      </section>
    </>
  );
}

export default Pricing;
